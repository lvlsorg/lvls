...
import {LSP4DigitalAssetMetadata} from "./LSP4DigitalAssetMetadata.sol";
...
import {NoExtensionFoundForFunctionSelector, InvalidFunctionSelector, InvalidExtensionAddress} from "../LSP17ContractExtension/LSP17Errors.sol";
...
import {LibLSP7Storage, LibLSP7} from "../libraries/LibLSP7Storage.sol";

// errors
import {LSP7CannotSendToSelf, LSP7AmountExceedsAuthorizedAmount, LSP7InvalidTransferBatch, LSP7AmountExceedsBalance, LSP7DecreasedAllowanceBelowZero, LSP7CannotUseAddressZeroAsOperator, LSP7TokenOwnerCannotBeOperator, LSP7CannotSendWithAddressZero, LSP7NotifyTokenReceiverContractMissingLSP1Interface, LSP7NotifyTokenReceiverIsEOA} from "@lukso/lsp-smart-contracts/contracts/LSP7DigitalAsset/LSP7Errors.sol";

// constants
import {_INTERFACEID_LSP1} from "@lukso/lsp-smart-contracts/contracts/LSP1UniversalReceiver/LSP1Constants.sol";
import {_TYPEID_LSP7_TOKENOPERATOR, _TYPEID_LSP7_TOKENSSENDER, _TYPEID_LSP7_TOKENSRECIPIENT} from "@lukso/lsp-smart-contracts/contracts/LSP7DigitalAsset/LSP7Constants.sol";
 ...
 */
contract LSP7DigitalAssetFacet is LSP4DigitalAssetMetadata, LSP7DigitalAssetCore, LSP17Extendable {
    // --- Token queries
    ...
    function revokeOperator(address operator, bytes memory operatorNotificationData) public virtual {
        ...
        bytes memory lsp1Data = abi.encode(msg.sender, 0, operatorNotificationData);
        _notifyTokenOperator(operator, lsp1Data);
    }
     ...
     * @inheritdoc ILSP7DigitalAsset
     */
    function getOperatorsOf(address tokenOwner) public view virtual returns (address[] memory) {
        LibLSP7Storage storage ds = LibLSP7.libLSP7Storage();
        return ds._operators[tokenOwner].values();
    }
    ...
    function transferBatch(
        ...
        if (fromLength != to.length || fromLength != amount.length || fromLength != force.length || fromLength != data.length) {
     ...
     */
    function increaseAllowance(address operator, uint256 addedAmount, bytes memory operatorNotificationData) public virtual {
        uint256 newAllowance = authorizedAmountFor(operator, msg.sender) + addedAmount;

        _updateOperator(msg.sender, operator, newAllowance, operatorNotificationData);
     ...
     */
    function decreaseAllowance(address operator, uint256 subtractedAmount, bytes memory operatorNotificationData) public virtual {
     ...
     */
    function _updateOperator(address tokenOwner, address operator, uint256 allowance, bytes memory operatorNotificationData) internal virtual {
        LibLSP7Storage storage ds = LibLSP7.libLSP7Storage();
        ...
        ds._operatorAuthorizedAmount[tokenOwner][operator] = allowance;

        if (allowance != 0) {
            ds._operators[tokenOwner].add(operator);
            emit AuthorizedOperator(operator, tokenOwner, allowance, operatorNotificationData);
        } else {
            ds._operators[tokenOwner].remove(operator);
            emit RevokedOperator(operator, tokenOwner, operatorNotificationData);
        }
    }
    ...
    function _burn(address from, uint256 amount, bytes memory data) internal virtual {
        ...
        emit Transfer({operator: msg.sender, from: from, to: address(0), amount: amount, force: false, data: data});
    ...
    function _spendAllowance(address operator, address tokenOwner, uint256 amountToSpend) internal virtual {
        LibLSP7Storage storage ds = LibLSP7.libLSP7Storage();
        uint256 authorizedAmount = ds._operatorAuthorizedAmount[tokenOwner][operator];
        ...
        _updateOperator({tokenOwner: tokenOwner, operator: operator, allowance: authorizedAmount - amountToSpend, operatorNotificationData: ""});
    }
    ...
    function _notifyTokenOperator(address operator, bytes memory lsp1Data) internal virtual {
        if (ERC165Checker.supportsERC165InterfaceUnchecked(operator, _INTERFACEID_LSP1)) {
            try ILSP1UniversalReceiver(operator).universalReceiver(_TYPEID_LSP7_TOKENOPERATOR, lsp1Data) {
                return;
            } catch {
                return;
            }
